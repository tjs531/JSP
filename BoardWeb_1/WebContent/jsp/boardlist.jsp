...
<%
	...
	try {
		...
		while(rs.next()) {							//rs: 커서(줄(DB테이블 튜플)을 가리키고 있음) , rs.next() 반환값은 boolean. 가리키고 있는 튜플에 값이 있다면 true
			int i_board = rs.getInt("i_board");		//getInt(int 형 컬럼명)
			String title = rs.getNString("title");	//getNString(String 형 컬럼명)
		...
		if(rs!=null) { try{ rs.close(); }catch(Exception e){} }				//열때는 con->ps->rs순서, 닫을때는 반대로.
